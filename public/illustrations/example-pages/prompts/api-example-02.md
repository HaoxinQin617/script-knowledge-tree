...
- 底部：用一句结论“对于重要任务，还要验证回答格式。例如要求模型返回一张表，后端就要检查字段是否齐全，而不是模型说什么都直接展示。”收束，不重复标题。
...
DRAWINGS
1. 还要准备失败路线。
2. 模型服务可能超时，也可能短时间没有额度。
3. 产品不能让用户一直看着转圈。
4. 常见做法是设置等待上限，失败后重试一次，仍然失败就切换备用模型或告诉用户稍后再试。
5. 对于重要任务，还要验证回答格式。例如要求模型返回一张表，后端就要检查字段是否齐全，而不是模型说什么都直接展示。
