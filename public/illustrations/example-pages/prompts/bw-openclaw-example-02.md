...
DRAWINGS
1. 例如你让它把一批发票录入财务软件，它需要逐张识别金额、找到正确输入框、提交后确认页面是否成功。
2. 如果其中一张模糊，它不应猜一个数字继续，而应把异常单独列出。
3. 电脑代理的价值，就体现在能识别这种真实环境里的分支。
