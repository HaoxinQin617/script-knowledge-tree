...
DRAWINGS
1. 例如同一个客服问题偶尔答错，不应立刻换模型。
2. 先检查相关制度是否进入 Context 窗口，再看 API 是否传丢字段，然后固定提示和随机参数，最后用一批没见过的问题测试泛化。
3. 按顺序排查，才能知道问题来自资料、连接、生成还是模型能力。
