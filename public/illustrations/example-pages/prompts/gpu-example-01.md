...
DRAWINGS
1. CPU 像几个能力很强的师傅，擅长处理复杂而不同的任务。
2. GPU 更像一间有很多工位的车间，每个工位做的事情不算复杂，但能同时开工。
3. 模型生成文字时，大量数字要一起计算，GPU 的这种并行能力很合适。
