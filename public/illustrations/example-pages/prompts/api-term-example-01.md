...
DRAWINGS
1. 大家听到最多的一句话可能是：这个软件接了某个大模型的 API。
2. 听起来像一项很深的技术，其实 API 解决的是一个朴素问题：两个互不认识的软件，怎样按固定方式交换信息。
3. 它不是模型，也不是聊天机器人，更像一扇写清楚办事规则的服务窗口。
