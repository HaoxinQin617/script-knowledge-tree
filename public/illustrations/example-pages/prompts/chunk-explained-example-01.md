---
id: chunk-explained-example-01
node_id: chunk-explained
language: zh-CN
aspect_ratio: "16:10"
backend: imagegen
reference_images: []
...
ZONES
- 顶部：只放简洁主题标题“Chunk 与重叠案例的完整逻辑”，禁止放正文长句或关系说明。
- 中部：按照实际逻辑放置 2 个清晰分区。每个分区包含一个短标题和一个能独立说明该步骤的简笔画小场景。
- 分区必须按阅读顺序由完整箭头串联；箭头头部较小，线条清晰，不得与卡片、文字或插画重叠。
- 底部：用一句结论“例如第一片包含第 1 到 5 段，第二片从第 5 段开始，这样跨边界的一句话不会被彻底拆散。”收束，不重复标题。
...
DRAWINGS
1. Chunk overlap 是相邻片段保留一小段重复内容。
2. 例如第一片包含第 1 到 5 段，第二片从第 5 段开始，这样跨边界的一句话不会被彻底拆散。
