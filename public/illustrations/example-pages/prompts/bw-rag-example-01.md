...
ZONES
- 顶部：只放简洁主题标题“RAG案例的完整逻辑”，禁止放正文长句或关系说明。
- 中部：按照实际逻辑放置 3 个清晰分区。每个分区包含一个短标题和一个能独立说明该步骤的简笔画小场景。
- 分区必须按阅读顺序由完整箭头串联；箭头头部较小，线条清晰，不得与卡片、文字或插画重叠。
- 底部：用一句结论“好的检索要按含义找到同一段，再让模型把规定换成自然回答，同时附上制度名称和页码。”收束，不重复标题。
...
DRAWINGS
1. 例如员工问差旅报销期限，资料里写的是费用发生后三十天内提交。
2. 用户可能问最晚什么时候交票，关键词并不相同。
3. 好的检索要按含义找到同一段，再让模型把规定换成自然回答，同时附上制度名称和页码。
