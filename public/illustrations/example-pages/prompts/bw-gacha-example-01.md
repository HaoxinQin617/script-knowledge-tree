...
DRAWINGS
1. 例如让模型想十个标题，变化越多越有价值；让模型从发票提取金额，就希望每次都返回同样字段。
2. 前者可以提高随机性，后者应使用明确格式并由程序核对。
3. 不是模型应该永远稳定或永远有创意，而是任务决定允许多大变化。
