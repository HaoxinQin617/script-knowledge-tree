---
id: bw-multimodal-example-02
node_id: bw-multimodal
language: zh-CN
aspect_ratio: "16:10"
backend: imagegen
reference_images: []
...
- 底部：用一句结论“小字看不清时，应提示用户补拍，而不是猜测。”收束，不重复标题。

LABELS
以下中文标签必须逐字准确、完整显示，不得截断、省略或生成乱码：
1. 用户上传一张菜单照片并问哪些菜含花生
2. 仅仅看见图片不够
3. 小字看不清时

DRAWINGS
1. 例如用户上传一张菜单照片并问哪些菜含花生，系统既要识别菜名，也要理解过敏问题，还可能结合餐厅说明。
2. 仅仅看见图片不够，模型需要把视觉内容和语言目标对应起来。
3. 小字看不清时，应提示用户补拍，而不是猜测。
