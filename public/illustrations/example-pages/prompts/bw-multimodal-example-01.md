...
- 底部：用一句结论“它能看发票照片、听会议录音、读屏幕截图，也能把一种形式转成另一种，例如根据图片写说明。”收束，不重复标题。
...
DRAWINGS
1. 可以把它想成一个同时拥有眼睛、耳朵和语言能力的助手。
2. 它能看发票照片、听会议录音、读屏幕截图，也能把一种形式转成另一种，例如根据图片写说明。
